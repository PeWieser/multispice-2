...
export default function Canvas() {
  ...
  const onWheel = (e: React.WheelEvent) => {
    ...
    if (e.ctrlKey || e.metaKey || !e.shiftKey) {
      ...
      st.setView({ x: st.view.x + e.deltaX / st.view.zoom, y: st.view.y + e.deltaY / st.view.zoom });
    }
  };
  ...
  const fitView = () => {
    const st = useEditor.getState();
    const wrap = wrapRef.current;
    if (!wrap || !st.doc.instances.length) return;
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    for (const i of st.doc.instances) {
      const b = instanceBounds(i);
    ...
    const pad = 60;
    const zoom = Math.min((wrap.clientWidth - pad * 2) / (maxX - minX || 1), (wrap.clientHeight - pad * 2) / (maxY - minY || 1), 2.4);
    st.setView({ zoom, x: minX - pad / zoom, y: minY - pad / zoom });
  };
