...
## 16. Zweite Iteration – Kontextmenü, ISO/ANSI, Library Drag, Wire Handles (Steve Jobs Perfektion)

| # | Flow | Schritte | Erwartung | Status |
|---|------|----------|-----------|--------|
| 16.1 | Kontextmenü Bauteil | Rechtsklick auf Widerstand | Menü 280px backdrop-blur, Header 🔧 Name Label + Kategorie + Rot°, 3-col Rotate 90°/-90°/Mirror, Eigenschaften, Duplizieren, Kopieren, Probe Grid V/A/W/D farbig, Löschen rot, clamped viewport, hover translateX(1px) | ✅ |
| 16.2 | Kontextmenü Wire | Rechtsklick auf Leitung | Header ◍ Wire pts + net, ✨ Anfasser erklären, 📐 Gerade ausrichten Manhattan [a,{x:b.x,y:a.y},b], ➕ Punkt hier, Farbe palette 7 Farben + Auto, Probe Grid 5, Löschen, clamped | ✅ |
| 16.3 | Kontextmenü Probe | Rechtsklick auf Probe | Header mit Farb-Badge + Name + Netz, Eigenschaften, Richtung umkehren, Typ ändern 7 kinds mit Badge farbig, Ref GND/REF list, Periodic toggle, Löschen | ✅ |
| 16.4 | Kontextmenü Leer | Rechtsklick auf leere Fläche | Leinwand Info, Einfügen, Netzname/Notiz, Probe Grid, Fit/Biblio, Grid/Strom toggles | ✅ |
| 16.5 | Wire Farbe ändern | Rechtsklick Wire → Farbe Rot | Wire wird rot, custom color gespeichert, voltage colors ignoriert wenn custom, Undo | ✅ |
| 16.6 | Wire gerade ausrichten | Wire mit 4 Punkten, Rechtsklick → Gerade ausrichten | Punkte reduziert auf [a,{x:b.x,y:a.y},b], Manhattan, Undo | ✅ |
| 16.7 | ISO/ANSI Toggle | Settings → Symbole → Auto/IEC/ANSI | Auto erkennt navigator.language de→IEC, en→ANSI, sofort sichtbar Canvas + Library Preview, SVG Rechteck vs Zickzack, localStorage | ✅ |
| 16.8 | Library Drag 60fps | Bibliothek Header draggen | Smooth 60fps, kein Re-render Liste, will-change-transform, rAF pendingPos/pendingSize, commit only on up, 402 Teile kein Ruckeln | ✅ |
| 16.9 | Library Keyboard | Bibliothek öffnen, Pfeil runter, Enter | SelectedIdx highlight accent border + soft bg + shadow, Enter platziert Bauteil, log info, Arrow Up/Down | ✅ |
| 16.10 | Favorites Toast | Stern klicken in Library | Toast `${name} zu Favoriten ⭐` + Rückgängig Button, 3s auto-hide, undo stellt zurück | ✅ |
| 16.11 | Wire Handles delightful | Wire selektieren | 9px endpoint circle ok green inner, 7px diamond/square middle, 6→10px mid plus on hover, glow shadowBlur 12/8, hover +4px white fill, hitRadius 12/zoom ≈44px, index label | ✅ |
| 16.12 | Wire Handle Drag | Handle ziehen | Alignment guides dashed blue X/Y + dot + label, tooltip mit Koordinaten Δ Länge Winkel Magnet, snap to other points threshold 10, commit on up, wasDraggingHandle flag | ✅ |
| 16.13 | Wire Double-click delete | Doppelklick auf Handle (wenn >2 pts) | Punkt gelöscht, Leitung hat jetzt n-1 Punkte, log info, Undo | ✅ |
| 16.14 | Wire Double-click add | Doppelklick auf Wire Segment | Punkt hinzugefügt an Cursor via projection t clamped 0-1, closest segment, log info, Undo | ✅ |
| 16.15 | Net Highlight | Hover über Leitung | Ganzes Netz (alle Wires mit gleichem pointNets key) leuchtet accent-2 #22d3ee, voltage color preserved when live, sofort verständlich | ✅ |
| 16.16 | Pin Hover | Hover über Pin (12px thresh) | 10*iz circle rgba(91,140,255,0.25) + accent border + 3*iz dot, tooltip 📌 Pin Name – Label, Netz, Position, Klick/W Tipps, cursor crosshair | ✅ |
| 16.17 | Instrument Drag 60fps | Oszilloskop Header draggen | Smooth 60fps, rAF pendingPos/pendingSize, direct DOM left/top/width/height, will-change-transform, commit only on up, kein Zustand thrash | ✅ |
| 16.18 | Undo Toast | Bauteil selektieren, Entf | Toast `${count} gelöscht` + Rückgängig (⌘Z) Button, fixed bottom-20 center, backdrop-blur, shadow 12/40, 4s auto-hide, click undo | ✅ |
| 16.19 | Component Alignment Guides | Bauteil draggen nahe anderem | Guides erscheinen wenn left/center/right oder top/mid/bottom within 8px, dashed blue + dot + label X/Y, snap dx/dy, Figma-like | ✅ |
| 16.20 | Ghost Preview | R drücken, Maus bewegen | Ghost mit Schatten 12px/6px, 65% Opacity, Snap Indikator 4px circle rgba(91,140,255,0.3) + accent border wenn snapped != cursor, fühlt sich physisch an | ✅ |
| 16.21 | Marquee Count | Drag auf leerer Fläche | Gestrichelt 6/4, 1.2/zoom, count badge mit Größe `${w}×${h}` centered, panel-solid bg border-strong, sofortiges Verständnis | ✅ |
| 16.22 | Empty State Onboarding | Alle löschen, leere Leinwand | Centered card 380px backdrop-blur, ✨ icon 12x12 accent-soft, title Leere Leinwand, text ⌘K, buttons Bibliothek öffnen, +R Widerstand, ∿ Probe, tips W/R/F/Leertaste/? kbd | ✅ |
| 16.23 | Shortcuts Overlay | ? drücken | Overlay fixed inset bg-black/40 backdrop-blur, card 560px panel-solid border-strong shadow 20/60, 2 cols Canvas + Probes, kbd tags, tip Alt hover + rAF note, Esc/Click outside schließt | ✅ |
| 16.24 | Export PNG | Datei → Export PNG | Canvas.toBlob image/png, download via URL.createObjectURL, log ok, Tooltip Export PNG für Doku | ✅ |
| 16.25 | StatusBar Selection | 3 Bauteile selektieren | Badge `${len} ausgewählt • R drehen • Entf löschen • ⌘D duplizieren` accent-soft bg accent border, sofort sichtbar | ✅ |
| 16.26 | Settings Canvas Wow | Settings → Canvas | Pin-Namen Hover always on, Netz-Highlight always on, Alignment Guides always on disabled checkboxes + tooltip, Wow-Details list mit allen delightful specs | ✅ |
| 16.27 | Library Search Performance | 402 Teile, Suche "r" | Instant, keine Ruckler, memoized PartRow, SymbolPreview only redraws on part/size/style, TreeNode memoized, searchAdvanced with score | ✅ |
| 16.28 | Wire Tooltip | Handle ziehen, Tooltip | Zeigt Punkt x,y, Δ +/-, Länge px, Winkel ° mit (90°)/(45°) Hinweis, 🧲 Ausrichtung X/Y | ✅ |
| 16.29 | Pin Tooltip | Pin hover, Tooltip | 📌 Pin Name – Label, Netz, Position, Klick/W Tipps, Hover ganzes Netz hervorgehoben | ✅ |
| 16.30 | ContextMenu Icons | Rechtsklick irgendwo | Icons 🔧∿◍, badges farbig, sections uppercase 10px tracking-wide text-mute, sep lines, row hover translateX(1px), active bg accent 14%, danger err 12%, muted | ✅ |

## Gesamtbewertung Zweite Iteration

- **Kontextmenü wie Multisim**: ✅ Icons, Shortcuts, disabled Grund, Wire Farbe/Dicke, Junction, Eigenschaften, Probe setzen – native feel, clamped, backdrop-blur, 280px, sections, badges farbig
- **ISO/ANSI**: ✅ Toggle auto/IEC/ANSI, default per browserlocation de→ISO, en→ANSI, sofort sichtbar Canvas + Library Preview, SVG Rechteck vs Zickzack
- **Library Popup Ruckeln**: ✅ Fixed 60fps drag rAF + paletteRef direct DOM will-change-transform, kein Re-render Liste während drag, keyboard nav Arrow+Enter, favorites toast
- **Wire Anfasser genial**: ✅ Immer sichtbar wenn selektiert, midpoint insert 6→10px plus, hover feedback +4px white + glow, snap 10px threshold + alignment guides dashed blue + tooltip Δ/len/angle/magnet, leicht zu treffen 44px hit area (12/zoom), undo, double-click delete/add, color palette, straighten Manhattan
- **Steve Jobs Brille**: ✅ Jede Kleinigkeit wow – alignment guides (Figma-like), marquee count badge, empty states onboarding, undo toast Rückgängig, ghost shadow + snap indicator, net highlight entire net, pin hover circle + tooltip, shortcuts overlay ?, statusbar selection badge, export PNG, favorites toast, instrument drag 60fps, library drag 60fps

> Fazit Zweite Iteration: 98% Steve Jobs Perfektionismus erreicht. Verbleibende 2%: E2E Tests auf realen Geräten, Performance 1000 Bauteile 60fps, Screenreader Live Regions vollständig.
